...
struct MyCubicKeyframe<Value: Animatable>: MyKeyframes {
    var initialVelocity: Value? // todo
    var endVelocity: Value? // todo
    var to: Value
    var duration: TimeInterval
    ...
    func interpolate(from: Value, time: TimeInterval) -> Value {
        let progress = time/duration
        let bezier = CubicBezier(p0: AnimatablePair(0, from.animatableData), p1: AnimatablePair(1/3.0, from.animatableData), p2: AnimatablePair(2/3.0, to.animatableData), p3: AnimatablePair(1, to.animatableData))
        var result = from
